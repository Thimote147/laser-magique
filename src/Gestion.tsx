						...
						>
							<div className="w-full flex items-center justify-between">
								<motion.h2 className="text-4xl mb-2 font-bold tracking-wider text-zinc-300">
									{new Date().toLocaleString('fr-FR', { month: 'long' }).charAt(0).toUpperCase() + new Date().toLocaleString('fr-FR', { month: 'long' }).slice(1)} <span className="opacity-50">{new Date().getFullYear()}</span>
								</motion.h2>
								<motion.button
							...
							>
								<div className="w-full flex flex-col items-start justify-between">
									...
									<p className="font-medium text-zinc-300/50">
										{hoveredDay
											? `Réservations pour le ${hoveredDay} Nov`
											: "Sélectionnez une date pour afficher les réservations"}
									</p>
								</div>
								<motion.div
												...
												>
													{day.meetingInfo &&
														day.meetingInfo.map((meeting, mIndex) => (
															<motion.div
																key={mIndex}
																className={`p-3 border-b last:border-b-0 border-[#323232] ${hoveredDay === day.day ? 'bg-[#1e1e1e]' : ''}`}
...
const DAYS: DayType[] = (() => {
	...
	for (let i = 0; i < daysInMonth; i++) {
		...
		days.push({
			day,
			classNames: "bg-[#1e1e1e]" + (dayOfWeek === 5 || dayOfWeek === 6 ? " bg-zinc-700/20" : ""),
			meetingInfo: i % 5 === 0 ? [
				{
					date: `Day ${day}`,
